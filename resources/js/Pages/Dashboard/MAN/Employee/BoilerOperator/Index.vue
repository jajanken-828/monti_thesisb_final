...
import { Flame, Gauge, AlertTriangle, Sparkles, ChevronRight, ArrowUpRight, ListOrdered, Wrench } from 'lucide-vue-next';
...
<template>
    <AuthenticatedLayout>
        <Head title="Boiler Operator Dashboard" />
            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-blue-100">
                                <Sparkles class="h-3.5 w-3.5" /> MAN · Boiler Department
                            </p>
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Boiler Operator Dashboard</h1>
                            <p class="text-sm text-blue-100/90">{{ stats.boilers }} boiler units · {{ stats.boilers_down }} down · {{ stats.logs_today }} logs today</p>
                        </div>
                        <div class="flex flex-wrap items-center gap-2">
                            <Link :href="route('man.staff.boiler-operator.page')"
                            ...
                            <Link :href="route('man.staff.boiler-operator.history')"
                            ...
                            <Link :href="route('man.staff.boiler-operator.reports')"
                                class="rounded-2xl bg-white/15 px-4 py-2.5 text-xs font-black uppercase tracking-wide text-white ring-1 ring-white/25 backdrop-blur hover:bg-white/25 active:scale-95 transition-all duration-200">
                                View Reports
                            </Link>
                        </div>
                ...
                <div v-if="efficiency" class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm p-5" style="animation-delay: 120ms">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-center">
                        ...
                        <div class="rounded-2xl bg-rose-50/70 dark:bg-rose-950/30 p-3"><p class="text-2xl font-black text-rose-700 dark:text-rose-300">{{ efficiency.machines?.down ?? 0 }}/{{ efficiency.machines?.total ?? 0 }}</p><p class="text-[10px] font-bold uppercase tracking-wide text-gray-500">Boilers down</p></div>
                    </div>
                </div>

                <TransitionGroup name="card" tag="div" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div key="boilers" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Boiler Units</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ stats.boilers }}</p>
                            </div>
                    ...
                    <div key="logs" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Logs Today</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ stats.logs_today }}</p>
                            </div>
                    ...
                    <div key="reports" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">My Open Reports</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ efficiency?.my_open_reports ?? 0 }}</p>
                            </div>
                        </div>
                        <ArrowUpRight class="absolute top-4 right-4 h-4 w-4 text-indigo-300 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 160ms">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-3">
                        ...
                        <h2 class="text-sm font-black text-gray-900 dark:text-white tracking-tight">My Recent Boiler Logs</h2>
                        <span class="ml-auto text-[9px] font-black uppercase px-2.5 py-1 rounded-full ring-1 bg-indigo-100 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30 flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> {{ recentLogs.length }} logs
                        </span>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50/80 dark:bg-zinc-800/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Boiler</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Pressure</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Date</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                                <tr v-for="log in recentLogs" :key="log.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                    <td class="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">{{ log.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ log.machine?.machine_no }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ log.steam_pressure ?? '—' }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ new Date(log.processed_at).toLocaleString() }}</td>
                                </tr>
                                    ...
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            <div class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-indigo-900/30 rounded-full mb-3 animate-bounce-soft">
                                                <Wrench class="h-7 w-7 text-indigo-400" />
                                            </div>
                                            <p class="text-sm font-black text-gray-700 dark:text-gray-200">No boiler logs recorded yet.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
