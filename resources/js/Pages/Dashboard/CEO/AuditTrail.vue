            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                ...
                <div class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm p-4">
                    <div class="relative">
                        ...
                        <input v-model="search" placeholder="Search action, reason, person…" class="w-full rounded-2xl border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 pl-9 pr-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto"><table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                            <tr v-for="log in logs?.data ?? []" :key="log.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                <td class="px-6 py-4 text-xs text-gray-500 whitespace-nowrap">{{ fmtDate(log.created_at) }}</td>
                                <td class="px-6 py-4"><span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase ring-1 bg-indigo-50 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30">{{ log.action }}</span></td>
                                <td class="px-6 py-4 text-sm font-bold text-gray-900 dark:text-white">{{ log.admin?.name ?? `#${log.admin_id}` }}<p class="text-[11px] font-medium text-gray-400">{{ log.admin?.email ?? '' }}</p></td>
                                <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{{ log.target?.name ?? log.target_name ?? '—' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" :title="log.reason">{{ log.reason ?? '—' }}</td>
                            </tr>
                            <tr v-if="!logs?.data?.length"><td colspan="5" class="px-6 py-12 text-center text-sm text-gray-400">No audit entries found.</td></tr>
                        </tbody>
                    </table></div>
                ...
                <div class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-2">
                        ...
                        <span class="ml-auto text-[11px] text-gray-400">from Approvals Center · tamper-proof</span>
                    </div>
                    <div class="overflow-x-auto"><table class="w-full">
                        <thead class="bg-gray-50/80 dark:bg-zinc-800/50"><tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">When</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                            <tr v-for="d in executiveDecisions ?? []" :key="d.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                <td class="px-6 py-3 text-xs text-gray-500 whitespace-nowrap">{{ fmtDate(d.created_at) }}</td>
                                <td class="px-6 py-3"><span :class="d.decision === 'approved' ? 'bg-emerald-100 text-emerald-700 ring-emerald-200' : 'bg-rose-100 text-rose-700 ring-rose-200'" class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase ring-1">{{ d.action_type }} · {{ d.decision }}</span></td>
                                <td class="px-6 py-3 text-sm font-bold">{{ d.subject_label }}<p v-if="d.reason" class="text-[11px] font-medium text-gray-400">Reason: {{ d.reason }}</p></td>
                                <td class="px-6 py-3 text-sm text-gray-600 dark:text-gray-300">{{ d.actor?.name ?? '—' }}</td>
                            </tr>
                            <tr v-if="!executiveDecisions?.length"><td colspan="4" class="px-6 py-8 text-center text-xs text-gray-400">No executive decisions recorded yet.</td></tr>
                        </tbody>
                    </table></div>
                </div>
            </div>
