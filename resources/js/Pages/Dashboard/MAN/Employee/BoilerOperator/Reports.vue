...
const submitReport = () => {
    form.post(route('man.staff.boiler-operator.report-machine'), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <AuthenticatedLayout title="Boiler Reports">
                ...
                <div class="animate-fade-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 sm:p-8 text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Boiler Reports</h1>
                            ...
                            <p class="text-sm text-blue-100/90">{{ myReports?.length ?? 0 }} report{{ (myReports?.length ?? 0) !== 1 ? 's' : '' }} · {{ machines?.length ?? 0 }} boiler units</p>
                        </div>
                ...
                <Transition name="modal">
                    <div v-if="showReportForm" class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                        <div class="bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 px-6 py-4">
                            <h2 class="flex items-center gap-2 text-lg font-black text-white tracking-tight"><AlertTriangle class="h-5 w-5" /> Report a Boiler Issue</h2>
                        ...
                        <form @submit.prevent="submitReport" class="space-y-4 p-6">
                            <div>
                                <label class="block text-xs font-black uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1.5">Boiler Unit</label>
                            ...
                            <div>
                                ...
                                <textarea v-model="form.issue" rows="3" required
                                    class="w-full rounded-2xl border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-3 text-sm font-medium focus:ring-2 focus:ring-indigo-500 outline-none transition"
                                    placeholder="Describe the fault, leak, pressure anomaly…"></textarea>
                            </div>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 100ms">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 text-left text-[10px] font-black uppercase tracking-wider text-gray-400">Boiler</th>
                        ...
                        <div v-if="!myReports || myReports.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Boiler issues you report will appear here.</p>
                        </div>
                    </div>
                </div>
            </div>
