...
import { ShieldCheck, Siren, CheckCircle2, Sparkles, ChevronRight, ArrowUpRight, ListOrdered, ClipboardList } from 'lucide-vue-next';
...
<template>
    <AuthenticatedLayout>
        <Head title="Safety Officer Dashboard" />
            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-blue-100">
                                <Sparkles class="h-3.5 w-3.5" /> MAN · Safety Office
                            </p>
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Safety Officer Dashboard</h1>
                            <p class="text-sm text-blue-100/90">{{ stats.open_incidents }} open cases · {{ stats.reported_today }} reported today</p>
                        </div>
                        <div class="flex flex-wrap items-center gap-2">
                            <Link :href="route('man.staff.safety-officer.page')"
                            ...
                            <Link :href="route('man.staff.safety-officer.history')"
                ...
                <div v-if="efficiency" class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm p-5" style="animation-delay: 120ms">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                        ...
                        <div class="rounded-2xl bg-emerald-50/70 dark:bg-emerald-950/30 p-3"><p class="text-2xl font-black text-emerald-700 dark:text-emerald-300">{{ stats.closed_total }}</p><p class="text-[10px] font-bold uppercase tracking-wide text-gray-500">Cases closed</p></div>
                    </div>
                </div>

                <TransitionGroup name="card" tag="div" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div key="open" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Open Cases</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ stats.open_incidents }}</p>
                            </div>
                    ...
                    <div key="today" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Reported Today</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ stats.reported_today }}</p>
                            </div>
                    ...
                    <div key="closed" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Cases Closed</p>
                                <p class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{{ stats.closed_total }}</p>
                            </div>
                        </div>
                        <ArrowUpRight class="absolute top-4 right-4 h-4 w-4 text-indigo-300 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 160ms">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-3">
                        ...
                        <h2 class="text-sm font-black text-gray-900 dark:text-white tracking-tight">My Recent Reports</h2>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50/80 dark:bg-zinc-800/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Type / Severity</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Status</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Date</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                                <tr v-for="i in recentIncidents" :key="i.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                    <td class="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">{{ i.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ i.incident_type.replace('_', ' ') }} · {{ i.severity }}</td>
                                    <td class="px-6 py-4"><span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase ring-1 bg-indigo-50 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30">{{ i.status }}</span></td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ new Date(i.incident_date).toLocaleDateString() }}</td>
                                </tr>
                                    ...
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            <div class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-indigo-900/30 rounded-full mb-3 animate-bounce-soft">
                                                <ShieldCheck class="h-7 w-7 text-indigo-400" />
                                            </div>
                                            <p class="text-sm font-black text-gray-700 dark:text-gray-200">No incidents filed yet. Zero harm is the goal.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
