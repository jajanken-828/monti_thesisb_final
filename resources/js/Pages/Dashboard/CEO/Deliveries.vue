...
const badge = (s) => ({
    pending: 'bg-slate-100 text-slate-600', dispatched: 'bg-blue-100 text-blue-700',
    in_transit: 'bg-indigo-100 text-indigo-700', delivered: 'bg-emerald-100 text-emerald-700',
                ...
                <div class="relative overflow-hidden rounded-3xl bg-slate-900 p-6 text-white shadow-xl sm:p-7">
                    ...
                    <div class="relative mt-5 flex flex-col gap-2 sm:flex-row">
                        ...
                        <select v-model="status" @change="search"
                            class="rounded-xl border-0 bg-white/95 px-4 py-3 text-xs font-black uppercase text-gray-700 outline-none">
                    ...
                    <div v-for="d in deliveries" :key="d.id"
                        ...
                        <button @click="expanded = expanded === d.id ? null : d.id"
                            ...
                            <span class="min-w-0 flex-1">
                                ...
                                <span class="block truncate text-[11px] font-bold text-slate-400">{{ d.client_name }} · {{ d.truck }} · {{ d.driver }}</span>
                            </span>
                            ...
                            <ChevronDown class="h-4 w-4 shrink-0 text-slate-400 transition" :class="expanded === d.id ? 'rotate-180' : ''" />
                        </button>
                        <div v-if="expanded === d.id" class="grid gap-3 border-t border-slate-100 bg-slate-50/60 px-5 py-4 text-xs dark:border-zinc-800 dark:bg-zinc-800/40 sm:grid-cols-3">
                            <div class="space-y-1.5">
                                ...
                                <p><span class="font-bold text-slate-400">JO:</span> <span class="font-black">{{ d.jo_number || '—' }}</span></p>
                                <p><span class="font-bold text-slate-400">PO:</span> <span class="font-black">{{ d.po_number || '—' }}</span></p>
                                <p class="flex items-center gap-1.5"><CreditCard class="h-3.5 w-3.5 text-slate-400" />
                                    <span class="font-black" :class="d.po_payment === 'paid' ? 'text-emerald-600' : 'text-amber-600'">{{ d.po_payment || 'unpaid' }}</span></p>
                                <p><span class="font-bold text-slate-400">Route:</span> <span class="font-bold">{{ d.route }}</span></p>
                            </div>
                            <div class="space-y-1.5">
                                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Movement</p>
                                <p class="flex items-center gap-1.5"><Clock class="h-3.5 w-3.5 text-slate-400" /> Sched: <strong>{{ ftime(d.scheduled_departure) }}</strong></p>
                                ...
                                <p v-if="d.pod_notes" class="text-slate-500">“{{ d.pod_notes }}”</p>
                                <p v-if="d.client_confirmed" class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-0.5 text-[10px] font-black uppercase text-emerald-700">
                                    <CheckCircle2 class="h-3 w-3" /> Client confirmed receipt
                                </p>
                            </div>
                            <div class="space-y-1.5">
                                <p class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400"><Package class="h-3.5 w-3.5" /> Packages ({{ d.packages_count }})</p>
                                ...
                                <p class="flex items-center gap-1.5 pt-1"><User class="h-3.5 w-3.5 text-slate-400" /> Driver: <strong>{{ d.driver }}</strong></p>
                            </div>
                        </div>
                    </div>
                </div>
                <div v-else class="rounded-3xl border border-dashed border-gray-200 bg-white py-16 text-center dark:border-zinc-800 dark:bg-zinc-900">
