...
import { Scissors, AlertCircle, X, Sparkles, ClipboardList, ChevronRight } from 'lucide-vue-next';
    ...
    <AuthenticatedLayout title="Dyeing Forming">
        <div class="min-h-screen bg-gradient-to-b from-slate-50 via-white to-blue-50/40 dark:from-zinc-950 dark:via-zinc-950 dark:to-indigo-950/30">
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-blue-100">
                                <Sparkles class="h-3.5 w-3.5" /> MAN · Dyeing Forming
                            </p>
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Forming Workspace</h1>
                            <p class="text-sm text-blue-100/90">{{ ironJobs?.length ?? 0 }} iron job{{ (ironJobs?.length ?? 0) !== 1 ? 's' : '' }} ready for forming</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold ring-1 ring-white/25 backdrop-blur">
                                {{ ironJobs?.length ?? 0 }} ready
                            </span>
                ...
                <!-- Empty state -->
                <div v-if="!ironJobs || ironJobs.length === 0"
                    ...
                    <p class="text-sm font-black text-gray-700 dark:text-gray-200">No iron jobs ready</p>
                    <p class="text-xs text-gray-400 mt-1">Jobs will appear here once ironing is complete.</p>
                </div>

                <!-- Job grid -->
                <TransitionGroup v-else name="card" tag="div" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div v-for="(job, i) in ironJobs" :key="job.id"
                        ...
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="h-12 w-12 rounded-2xl bg-gradient-to-br from-orange-500 via-amber-600 to-rose-600 flex items-center justify-center text-white text-lg font-black shadow-lg uppercase flex-shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                                    {{ (job.code ?? '?').charAt(0) }}
                                </div>
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase truncate">
                                        Fabric: {{ job.squeezer_job?.softener_job?.fabric?.code || 'N/A' }}
                                    </p>
                                </div>
                            </div>
                            <span class="text-[9px] font-black uppercase px-2.5 py-1 rounded-full ring-1 flex-shrink-0 ml-2 flex items-center gap-1 bg-blue-100 text-blue-700 ring-blue-200 dark:bg-blue-500/15 dark:text-blue-300 dark:ring-blue-500/30">
                                <span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> Ready
                            </span>
                        </div>

                        <div class="space-y-1.5 mb-4 text-[12px]">
                            <p class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Product:</span> <span class="truncate font-medium">{{ job.product?.name || 'Not assigned' }}</span></p>
                            <p class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Weight:</span> <span class="truncate font-medium">{{ job.squeezer_job?.softener_job?.fabric?.weight }} kg</span></p>
                            <p class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Operator:</span> <span class="truncate font-medium">{{ job.operator?.name }}</span></p>
                            <p class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Shift:</span> <span class="truncate font-medium">{{ job.shift }}</span></p>
                            <p class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Date:</span> <span class="truncate font-medium">{{ new Date(job.processed_at).toLocaleString() }}</span></p>
                            <p v-if="job.remarks" class="flex items-center gap-2 text-gray-500 dark:text-gray-400"><span class="font-bold text-gray-700 dark:text-gray-200">Remarks:</span> <span class="truncate font-medium">{{ job.remarks }}</span></p>
                        </div>

                        <div class="mt-auto pt-3 border-t border-gray-100 dark:border-zinc-800">
                            <button @click="openFormModal(job)"
                                class="w-full bg-gradient-to-r from-orange-600 to-rose-600 hover:from-orange-700 hover:to-rose-700 text-white py-2.5 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200 active:scale-95">
            ...
            <div v-if="showFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative bg-white/95 dark:bg-zinc-900/95 backdrop-blur rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100 dark:border-zinc-800 animate-pop max-h-[90vh] flex flex-col">
                    <div class="relative overflow-hidden bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 text-white">
                        ...
                        <div class="relative flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-lg font-black tracking-tight">Record Forming</h3>
                                    <p class="font-mono text-xs text-blue-100">{{ currentIronJob?.code }}</p>
                                </div>
                            </div>
                    ...
                    <form @submit.prevent="submitForm" class="p-6 space-y-4 overflow-y-auto">
                        <div class="bg-indigo-50/60 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/40 p-3 rounded-2xl">
                            <p class="text-sm font-bold text-gray-700 dark:text-gray-200">Iron Job: <span class="font-mono">{{ currentIronJob?.code }}</span></p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Fabric: {{ currentIronJob?.squeezer_job?.softener_job?.fabric?.code }}</p>
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-black uppercase tracking-wide text-gray-600 dark:text-gray-300 mb-1.5">Product</label>
                        ...
                        <div>
                            <label class="block text-xs font-black uppercase tracking-wide text-gray-600 dark:text-gray-300 mb-1.5">Quantity</label>
                        ...
                        <div class="flex gap-3 pt-2">
                            <button type="submit" :disabled="form.processing"
                                class="flex-1 bg-gradient-to-r from-orange-600 to-rose-600 hover:from-orange-700 hover:to-rose-700 text-white py-2.5 rounded-2xl font-bold transition active:scale-95 disabled:opacity-50">
