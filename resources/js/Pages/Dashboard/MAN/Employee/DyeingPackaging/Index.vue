...
import { Box, Package, Clock, Sparkles, ChevronRight, ArrowUpRight, ListOrdered } from 'lucide-vue-next';
            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                ...
                <div class="animate-fade-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 sm:p-8 text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Packaging Dashboard</h1>
                            <p class="text-sm text-blue-100/90">{{ stats.pending }} available products · {{ stats.total_today }} packages today</p>
                        </div>
                    ...
                    <div class="relative mt-6 flex flex-wrap items-center gap-2">
                        <span class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold ring-1 ring-white/25 backdrop-blur flex items-center gap-1.5">
                            <span class="h-1.5 w-1.5 rounded-full bg-emerald-300 animate-pulse" /> {{ stats.pending }} available
                        </span>
                        <span class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold ring-1 ring-white/25 backdrop-blur flex items-center gap-1.5">
                            <span class="h-1.5 w-1.5 rounded-full bg-cyan-300 animate-pulse" /> {{ stats.total_today }} today
                        </span>
                    </div>
                </div>
                ...
                <TransitionGroup name="card" tag="div" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div key="pending" style="transition-delay: 0ms"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Available Products</p>
                    ...
                    <div key="today" style="transition-delay: 40ms"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Packages Today</p>
                    ...
                    <div key="shipments" style="transition-delay: 80ms"
                        class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/15 hover:-translate-y-1.5 hover:border-indigo-200 dark:hover:border-indigo-800 hover:scale-[1.01] transition-all duration-300 p-5 overflow-hidden">
                        <div class="pointer-events-none absolute -top-16 -right-16 h-40 w-40 rounded-full bg-gradient-to-br from-indigo-400/20 to-fuchsia-400/20 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                        <div class="absolute left-0 top-5 bottom-5 w-1 bg-gradient-to-b from-indigo-500 to-fuchsia-500 rounded-r-full scale-y-0 group-hover:scale-y-100 origin-center transition-transform duration-300" />
                        <div class="flex items-center gap-3">
                            <div class="h-12 w-12 rounded-2xl bg-gradient-to-br from-violet-500 via-purple-600 to-fuchsia-700 flex items-center justify-center text-white shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                                <Box class="h-6 w-6" />
                            </div>
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Pending Shipments</p>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 160ms">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-3">
                        <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 dark:bg-emerald-900/20"><ListOrdered class="h-4 w-4 text-emerald-500" /></span>
                        <h2 class="text-sm font-black text-gray-900 dark:text-white tracking-tight">Recent Packages</h2>
                        <span class="ml-auto text-[9px] font-black uppercase px-2.5 py-1 rounded-full ring-1 bg-indigo-100 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30 flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> {{ recentPackages.length }} packages
                        </span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                                <tr v-for="pkg in recentPackages" :key="pkg.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                    <td class="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">{{ pkg.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ pkg.items?.length }} items</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ new Date(pkg.packaged_at).toLocaleString() }}</td>
                                    <td class="px-6 py-4"><span
                                            :class="pkg.status === 'pending' ? 'bg-amber-100 text-amber-700 ring-amber-200 dark:bg-amber-500/15 dark:text-amber-300 dark:ring-amber-500/30' : 'bg-emerald-100 text-emerald-700 ring-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:ring-emerald-500/30'"
                                            class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase ring-1 flex items-center gap-1 w-fit"><span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" />{{ pkg.status }}</span></td>
                                </tr>
                                <tr v-if="recentPackages.length === 0">
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm font-black text-gray-700 dark:text-gray-200">No packages yet.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
