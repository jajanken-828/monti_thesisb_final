        ...
        <div class="min-h-screen bg-gradient-to-b from-slate-50 via-white to-blue-50/40 dark:from-zinc-950 dark:via-zinc-950 dark:to-indigo-950/30">
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                <div class="animate-fade-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 sm:p-8 text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <div class="relative mt-6 flex flex-wrap gap-2">
                        <button v-for="t in tabs" :key="t.key" @click="tab = t.key"
                            :class="['flex items-center gap-1.5 rounded-2xl px-4 py-2.5 text-xs font-black uppercase tracking-wide transition-all', tab === t.key ? 'bg-white text-indigo-700 shadow-lg' : 'bg-white/15 text-white ring-1 ring-white/25 backdrop-blur hover:bg-white/25']">
                ...
                <div v-if="tab === 'payroll'" class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                    <div class="overflow-x-auto"><table class="w-full">
                        <thead class="bg-gray-50/80 dark:bg-zinc-800/50"><tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Decision</th>
                        </tr></thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                            <tr v-for="p in payrolls" :key="p.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20">
                                <td class="px-6 py-4 text-sm font-bold">{{ p.employee_name ?? `#${p.employee_id}` }}<p class="text-[11px] font-medium text-gray-400">Payroll #{{ p.id }} · {{ p.days_worked ?? '—' }} days</p></td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    <div v-if="reasonFor === `payroll-${p.id}`" class="mt-2 flex gap-2 justify-end">
                                        <input v-model="reason" placeholder="Rejection reason (required)" class="rounded-xl border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-rose-400 w-56" />
                            ...
                            <tr v-if="!payrolls?.length"><td colspan="3" class="px-6 py-10 text-center text-sm text-gray-400">No pending payroll. Queue clear.</td></tr>
                        </tbody>
                    </table></div>
                </div>
                    ...
                    <div class="overflow-x-auto"><table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                            <tr v-for="v in vendors" :key="v.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20">
                                <td class="px-6 py-4 text-sm font-bold">{{ v.business_name }}<p class="text-[11px] font-medium text-gray-400">{{ v.email }}</p></td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ v.representative_name ?? '—' }}<p class="text-[11px] text-gray-400">{{ v.phone_number ?? '' }}</p></td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                            ...
                            <tr v-if="!vendors?.length"><td colspan="3" class="px-6 py-10 text-center text-sm text-gray-400">No pending vendor registrations.</td></tr>
                        </tbody>
                    </table></div>
                </div>
                ...
                <div v-if="tab === 'credits'" class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                    <div class="overflow-x-auto"><table class="w-full">
                        <thead class="bg-gray-50/80 dark:bg-zinc-800/50"><tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">PO / Client</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Amount</th>
                            <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Decision</th>
                        </tr></thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                            <tr v-for="o in credits" :key="o.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20">
                                <td class="px-6 py-4 text-sm font-bold font-mono">{{ o.po_number }}<p class="font-sans text-[11px] font-medium text-gray-400">{{ o.client }}</p></td>
                            ...
                            <tr v-if="!credits?.length"><td colspan="3" class="px-6 py-10 text-center text-sm text-gray-400">No orders pending credit review.</td></tr>
                        </tbody>
                    </table></div>
                </div>
                ...
                <div v-if="tab === 'vacancies'" class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm p-6">
                    ...
                    <p class="text-xs text-gray-500 mb-4">Appoint from Manufacturing → Access Control (personnel-governance exception to the overseer model).</p>
                    <div v-if="vacancies?.length" class="flex flex-wrap gap-2">
                        <span v-for="d in vacancies" :key="d" class="rounded-full bg-amber-100 text-amber-800 ring-1 ring-amber-200 dark:bg-amber-500/15 dark:text-amber-300 dark:ring-amber-500/30 px-4 py-1.5 text-xs font-black uppercase">{{ d }} vacant</span>
                    </div>
                    <p v-else class="text-sm text-emerald-600 font-bold">All department supervisor seats are filled.</p>
                </div>
                ...
                <div class="bg-white/80 dark:bg-zinc-900/80 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                    ...
                    <ul class="divide-y divide-gray-100 dark:divide-zinc-800">
                        <li v-for="d in recentDecisions" :key="d.id" class="px-6 py-3 text-sm flex flex-wrap items-center gap-x-3 gap-y-1">
                            <span :class="d.decision === 'approved' ? 'bg-emerald-100 text-emerald-700 ring-emerald-200' : 'bg-rose-100 text-rose-700 ring-rose-200'" class="px-2 py-0.5 rounded-full text-[10px] font-black uppercase ring-1">{{ d.decision }}</span>
                            <span class="font-bold">{{ d.subject_label }}</span>
                            <span class="text-xs text-gray-400 ml-auto">{{ d.actor?.name }} · {{ new Date(d.created_at).toLocaleString() }}</span>
                            <p v-if="d.reason" class="w-full text-xs text-gray-500">Reason: {{ d.reason }}</p>
                        </li>
                        <li v-if="!recentDecisions?.length" class="px-6 py-8 text-center text-xs text-gray-400">No decisions recorded yet.</li>
                    </ul>
                </div>
            </div>
        </div>
