...
import { Droplet, Clock, AlertTriangle, Sparkles, ChevronRight, ArrowUpRight, ListOrdered } from 'lucide-vue-next';
            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                ...
                <div class="animate-fade-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 sm:p-8 text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Dyeing Color Dashboard</h1>
                            <p class="text-sm text-blue-100/90">{{ stats.pending }} pending to dye · {{ stats.total_today }} done today</p>
                        </div>
                    ...
                    <div class="relative mt-6 flex flex-wrap items-center gap-2">
                        <span class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold ring-1 ring-white/25 backdrop-blur flex items-center gap-1.5">
                            <span class="h-1.5 w-1.5 rounded-full bg-fuchsia-300 animate-pulse" /> {{ stats.pending }} pending
                        </span>
                        <span class="rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold ring-1 ring-white/25 backdrop-blur flex items-center gap-1.5">
                ...
                <TransitionGroup name="card" tag="div" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div key="pending" style="transition-delay: 0ms"
                        ...
                        <div class="flex items-center gap-3">
                            <div class="h-12 w-12 rounded-2xl bg-gradient-to-br from-fuchsia-500 via-purple-600 to-indigo-700 flex items-center justify-center text-white shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                                <Droplet class="h-6 w-6" />
                            </div>
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Pending to Dye</p>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 160ms">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-3">
                        <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-fuchsia-50 dark:bg-fuchsia-900/20"><ListOrdered class="h-4 w-4 text-fuchsia-500" /></span>
                        <h2 class="text-sm font-black text-gray-900 dark:text-white tracking-tight">Recent Dye Jobs</h2>
                        <span class="ml-auto text-[9px] font-black uppercase px-2.5 py-1 rounded-full ring-1 bg-indigo-100 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30 flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> {{ recentJobs.length }} jobs
                        </span>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50/80 dark:bg-zinc-800/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Fabric Code</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Dye Type</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Date</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                                <tr v-for="job in recentJobs" :key="job.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                    <td class="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">{{ job.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ job.fabric?.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ job.dye_type }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ new Date(job.processed_at).toLocaleString() }}</td>
                                </tr>
                                    ...
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            <div class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-indigo-900/30 rounded-full mb-3 animate-bounce-soft">
                                                <Droplet class="h-7 w-7 text-indigo-400" />
                                            </div>
                                            <p class="text-sm font-black text-gray-700 dark:text-gray-200">No dye jobs yet.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
