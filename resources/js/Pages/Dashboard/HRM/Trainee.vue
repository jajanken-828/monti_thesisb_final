...
const canEdit = computed(() => {
    ...
    if (props.permissions?.trainee === 'edit') return true;
...
if (page.props.flash?.error) triggerToast(page.props.flash.error);
...
// Open modals (only if can edit)
const openGradeModal = (trainee) => {
    if (!canEdit.value) {
        triggerToast('You do not have permission to grade trainees.');
        return;
    }
...
const openPassModal = (trainee) => {
    if (!canEdit.value) {
        triggerToast('You do not have permission to promote trainees.');
        return;
    }
    selectedTrainee.value = trainee;
...
const openFailModal = (trainee) => {
    if (!canEdit.value) {
        triggerToast('You do not have permission to fail trainees.');
        return;
    }
    selectedTrainee.value = trainee;
    failReason.value = '';
...
<template>
    <Head :title="`${departmentName} Trainee Development`" />
        ...
        <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8">
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    {{ departmentName }} Trainee <span class="text-blue-600">Development</span>
                </h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium">
                    Monitor and manage trainees from all modules across the company.
                </p>
            ...
            <div v-if="trainees.length === 0" class="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 p-12 text-center">
                ...
                <p class="text-sm text-slate-500 mt-2">All trainees across all modules will appear here for HRM to manage.</p>
            </div>
                ...
                <div v-for="trainee in trainees" :key="trainee.id" class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden hover:shadow-lg transition-all">
                    <div class="p-6">
                        <div class="flex items-start justify-between">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-bold text-blue-600 mt-1">Module: {{ trainee.role }}</p>
                                </div>
                            </div>
                        ...
                        <div v-else class="mt-6 text-center text-xs text-gray-400 italic">
                            View only mode – contact HRM for grading
                        </div>
