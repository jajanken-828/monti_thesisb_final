...
import { Cog, Clock, AlertTriangle, Sparkles, ChevronRight, ArrowUpRight, ListOrdered, Wrench } from 'lucide-vue-next';
...
<template>
    <AuthenticatedLayout>
        <Head title="Knitting Mechanic Dashboard" />
            ...
            <div class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 pb-16">
                ...
                <div class="animate-fade-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-700 to-violet-800 p-6 sm:p-8 text-white shadow-xl shadow-indigo-500/20">
                    ...
                    <div class="relative flex flex-wrap items-center gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Knitting Mechanic Dashboard</h1>
                            <p class="text-sm text-blue-100/90">{{ stats.pending }} open sheets · {{ stats.total_today }} recorded today</p>
                        </div>
                        <div class="flex flex-wrap items-center gap-2">
                            <Link :href="route('man.staff.knitting-mechanic.page')"
                            ...
                            <Link :href="route('man.staff.knitting-mechanic.history')"
                            ...
                            <Link :href="route('man.staff.knitting-mechanic.reports')"
                                class="rounded-2xl bg-white/15 px-4 py-2.5 text-xs font-black uppercase tracking-wide text-white ring-1 ring-white/25 backdrop-blur hover:bg-white/25 active:scale-95 transition-all duration-200">
                                View Reports
                            </Link>
                        </div>
                ...
                <TransitionGroup name="card" tag="div" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div key="pending" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Open Setup Sheets</p>
                    ...
                    <div key="today" class="group relative flex flex-col bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 p-5 overflow-hidden">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">Recorded Today</p>
                ...
                <div class="animate-fade-up bg-white/80 dark:bg-zinc-900/80 backdrop-blur rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden" style="animation-delay: 160ms">
                    <div class="px-6 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center gap-3">
                        ...
                        <h2 class="text-sm font-black text-gray-900 dark:text-white tracking-tight">My Recent Setup Sheets</h2>
                        <span class="ml-auto text-[9px] font-black uppercase px-2.5 py-1 rounded-full ring-1 bg-indigo-100 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30 flex items-center gap-1">
                            <span class="h-1.5 w-1.5 rounded-full bg-current animate-pulse" /> {{ recentSetups.length }} sheets
                        </span>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50/80 dark:bg-zinc-800/50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Task</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-zinc-800">
                                <tr v-for="s in recentSetups" :key="s.id" class="hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 transition-colors">
                                    <td class="px-6 py-4 font-mono text-sm text-gray-900 dark:text-white">{{ s.code }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ taskLabel(s.task_type) }}</td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ s.machine?.machine_no }}</td>
                                    <td class="px-6 py-4"><span class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase ring-1 bg-indigo-50 text-indigo-700 ring-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:ring-indigo-500/30">{{ s.status.replace('_', ' ') }}</span></td>
                                </tr>
                                <tr v-if="recentSetups.length === 0">
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            <div class="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-indigo-900/30 rounded-full mb-3 animate-bounce-soft">
                                                <Cog class="h-7 w-7 text-indigo-400" />
                                            </div>
                                            <p class="text-sm font-black text-gray-700 dark:text-gray-200">No setup sheets recorded yet.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
